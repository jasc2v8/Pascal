...
begin
  For i:=0 to ListBox1.Count-1 do begin
    ...
    if ListBox1.Selected[i] then
      ListBox1.Items[i]:=ListBox1.Items[i]+' ***** CLONED';
  end;
end;
...
begin
  ListBox1.Clear;
  SelectDirectoryDialog1.InitialDir:=SelectedDirectory;
  //SelectDirectoryDialog1.Filter:=('All VHD Files|*.hdd;*.vdi;*.vhd;*.vmdk|All Files|*.*');
  if SelectDirectoryDialog1.Execute then begin
    SelectedDirectory:=SelectDirectoryDialog1.FileName;
    try
      VhdFiles:=TStringList.Create;
      FindAllFiles(VhdFiles, SelectedDirectory, '*.hdd;*.vdi;*.vhd;*.vmdk', false);
      ListBox1.Items:=VhdFiles;
    finally
      VhdFiles.Free;
    end;
  end else
    ShowMessage('Selected Directory not found');

  ListBox1.Items[1]:=ListBox1.Items[1]+MDBUTTONBALLOTBOXWITHCHECK;
  ListBox1.Items[2]:=ListBox1.Items[2]+' ***** CLONED';
...
begin
  For i:=0 to ListBox1.Count-1 do begin
    if ListBox1.Selected[i] then begin
      ListBox1.Items[i]:=ListBox1.Items[i]+' ***** CLONING...';
      cmd:=VBManageWin64+' clonehd '+ListBox1.Items[i]+ListBox1.Items[i]+'_CLONED';
      ShowMessage('cmd='+cmd);
      Thread1:=TThreadUnit.Create(cmd, False);
