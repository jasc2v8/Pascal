{ Windows Auto Key v1.3.0

  v1.3  add GetVKeyboardState, SetVKeyboardState
  v1.2  add hCtl instance
  v1.1  add WAK.DetectHiddenWindows
...
var
  ...
  gCount: Integer=0;
  gTextBuf: String='';
  gCtrlClassNames: TStringList;
  gCtrlClassInstances: TStringList;
...
function GetVKeyboardState(const VK: Word=0): Boolean;
procedure SetVKeyboardState(const VK: Word=0; State: Word=VK_UP);
...
function hCtl(const h: HWND; const FindCtlText: string='';
  const FindCtlClass: string=''; const FindInstance: integer=1; const ID: integer=0): HWND;
...
function GetVKeyboardState(const VK: Word=0): Boolean;
var
  keys: array [0..256] of byte;
begin
  GetKeyboardState(keys);
  Result:=False;
  if keys[VK]=1 then Result:=True;
end;
//VK_UP = Key off, VK_DN = Key on
procedure SetVKeyboardState(const VK: Word=0; State: Word=VK_UP);
begin
  if (GetVKeyboardState(VK)) and (State=VK_UP) then
    Send(VK);
end;
...
begin
  ...
  WBuf:=''; //avoid the warning 'does not seem to be initialized'

  if (IsWindowVisible(h) or WAK.DetectHiddenWindows) then begin
    ...
    //if not already on the list then save as instance 1, else increment instance count
    i:=gCtrlClassNames.IndexOfName(CtlClassName);
    if i=-1 then begin
      gCtrlClassNames.Add(CtlClassName+'=1');
      gCtrlClassInstances.Add(CtlClassName+'1='+IntToStr(h));
      instance:=1;
    end else begin
      n:=gCtrlClassNames.Names[i];
      v:=gCtrlClassNames.Values[n];
      j:=StrToInt(v);
      Inc(j);
      gCtrlClassNames.Delete(i);
      gCtrlClassNames.Add(n+'='+IntToStr(j));
      gCtrlClassInstances.Add(n+IntToStr(j)+'='+IntToStr(h));
      instance:=j;
    end;
    ...
    if MatchID or
      ...
      (MatchClass and (instance=Ctl^.FindInstance) and (Ctl^.FindText='')) or
...
begin
  ...
  gCtrlClassNames:=TStringList.Create;
  gCtrlClassInstances:=TStringList.Create;

  Ctl.Handle:=0;
  Ctl.FindText:=FindCtlText;
  Ctl.FindClass:=FindCtlClass;
  Ctl.FindInstance:=FindInstance;
  Ctl.FindID:=ID;
  ...
  FreeAndNil(gCtrlClassNames);
  FreeAndNil(gCtrlClassInstances);
...
begin
  Result:=False;
  pt:=Default(Point); //avoid compiler warning
  try
...
begin
  ...
  if (IsWindowVisible(hc) or WAK.DetectHiddenWindows) then begin
...
initialization
  ...
  WAK.DetectHiddenWindows:=False; //False AutoHotKey
