...
//change WinSleep to Single so we can use WinSleep(1e3)

{ Windows Auto Key

0.1.0  Ready for alpha testing
0.0.99 Changed Delay to MilliSeconds
0.0.98 Removed DelayOverride in WinSleep(MilliSeconds)
0.0.97 Changed Settings to WAK
0.0.96 Removed DelayOverride for keyboard and mouse, kept in WinSleep, increased defaults
0.0.95 is WinSleep DelayOverride=0 working?
0.0.94 Change WinSleep DelayOverride: integer=-1
0.0.93 Change WinWait back to seconds
0.0.92 Add ControlClick
0.0.91 Add ControlGetPos
0.0.90 Cleanup some compiler errors by initialzing variables and using {H-}
0.0.89 Change WinWait* to delay first, then loop
0.0.88 Change WinGetText to EnumWindows, hWin works well
0.0.87 Testing hWin
0.0.86 Begin change hWin to EnumWindows - needs testing
0.0.85 Fixed hCtl - works well now
0.0.84 Change WinGetTextENUM to WinGetText, greatly simplified using gTextBuf
0.0.83 Fixed WinGetTextENUM using Windows sd call - ouch!
0.0.82 add WinGetTextENUM
0.0.81 Before fixing WinGetText to enumchildwindows
0.0.80 Finish hWin, hCtl.  Fixed WinSetState
0.0.79 Decided to go with WinFunction(h) and hWnd(), added ControlGetHandle and GetText
0.0.78 Begin to consider FindTitle: Variant, and ControlGetHandle
0.0.77 Change .LastFound to .LastUsed and update many functions
0.0.76 Change to winautokey_title_class_text, fix WinExist to WinExists
0.0.75 Archive as winautokey_title_text, change to (Title, Class, Text)
0.0.74 Add WinGetHandle - yeah!
0.0.73 Add WinGetText
0.0.72 Add _GetTag
0.0.71 Change WinGetHandleNEW to WinGetHandle, added Buttons
0.0.71 Change WinGetHandleNEW, reduce matches
0.0.70 Change WinGetHandleNEW so it returns child handle
0.0.69 WinGetHandleNEW tests OK
0.0.68 Change to [PARAMS]
0.0.67 Begin to add _ReadParams
0.0.66 Document specs for Params Array of String [1..8]
0.0.65 Change WinWait to use WinGetHandle
0.0.64 Change WinWaitActive to use WinGetHandle
0.0.63 Change WinWaitClose to use WinGetHandle
0.0.62 Change WinWait to use WinGetHandle
0.0.61 Change WinExist to use WinGetHandle
0.0.60 Begin to add Settings.LastUsed, needs work
0.0.59 Add 'ACTIVE' and 'LAST' to WinGetHandle
0.0.58 Change WinExistNEW to WinGetHandle
0.0.57 debug WinExistNEW
0.0.56 Fixed WinClose
0.0.55 Update Unit1
0.0.54 Add Settings
0.0.53 Verified to use Sleep() with Application.ProcessMessages
0.0.52 Add Mouse functions and eliminate MouseAndKeyInput
0.0.51 Changed GetKeyState, Fixed Send, add FWinDelay to Win functions,
...
{ Design concepts
  ...
  - Control: ClassNN
  - TextMatchMode: Slow or Fast (only Slow implemented)
...
procedure WinSleep(const MilliSeconds: Integer);
...
begin
  ...
  if IsWindowVisible(h) then begin
    ...
    if Win^.FindTitle<>'' then begin
      ...
      //Debugln('MatchTitle=',MatchTitle);

      //Debugln('FindClass='+Win^.FindClass);
      //Debugln('FindText='+Win^.FindText);

      if MatchTitle and (Win^.FindClass='') and (Win^.FindText='') then Result:=False;
    end else
    ...
    if Win^.FindClass<>'' then begin
      ...
      if MatchClass and MatchTitle and (Win^.FindText='') then Result:=False;
      if MatchClass and (Win^.FindTitle='') and (Win^.FindText='') then Result:=False;
    end else

    if Win^.FindText<>'' then begin
      WinText:=WinGetText(h);
      MatchText:=_DoMatch(WAK.TextMatchMode, Win^.FindText, WinText);
      if MatchText and MatchTitle and MatchClass then Result:=False;;
      if MatchText and (Win^.FindTitle='') and MatchClass then Result:=False;
      if MatchText and MatchTitle and (Win^.FindClass='') then Result:=False;
    end;
