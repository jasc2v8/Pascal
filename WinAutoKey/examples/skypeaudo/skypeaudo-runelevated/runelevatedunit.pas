unit RunElevatedUnit;
//http://forum.lazarus.freepascal.org/index.php?topic=22454.0 
// v1.0a (2016.01.02.)
// Forum member reported: IsElevated function is the exact copy from JCL,
// see:  https://github.com/project-jedi/jcl/blob/master/jcl/source/windows/JclSecurity.pas#L802
// v1.0 (2016.01.02.)
// Source from: http://stackoverflow.com/questions/923350/delphi-prompt-for-uac-elevation-when-needed
// Converted from Delphi code with Lazarus 1.4.4
// Thanks to rvk for the "ShellExecuteEx" -> "ShellExecuteExA" idea, so compile is success now.
// Irrelevant messages hide with directive.
...
begin
  ...
  try
    ...
    FileClose(SEI.hProcess); { *Converted from CloseHandle* }
  end;
end;
  ...
  try
    ...
    begin
      Win32Check(AllocateAndInitializeSid(SECURITY_NT_AUTHORITY, 2,
        SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0,
        psidAdmin));
      if GetTokenInformation(Token, TokenGroups, nil, 0, Count{%H-}) or
    ...
    if TokenInfo <> nil then
      FreeMem(TokenInfo);
    if HaveToken then
      FileClose(Token); { *Converted from CloseHandle* }
    if psidAdmin <> nil then
      FreeSid(psidAdmin);
  end;
end;
